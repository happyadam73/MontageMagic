...
+ (BOOL)unzipFontFilesAtPath:(NSString *)path toDestination:(NSString *)destination
{
	return [self unzipFontFilesAtPath:path toDestination:destination overwrite:YES password:nil error:nil];
}

+ (BOOL)unzipFontFilesAtPath:(NSString *)path toDestination:(NSString *)destination overwrite:(BOOL)overwrite password:(NSString *)password error:(NSError **)error 
{
	// Begin opening
	zipFile zip = unzOpen((const char*)[path UTF8String]);	
	if (zip == NULL) {
	...
	do {
        ...
        //check to see if it's a font (ttf or otf extension)
        NSString *extension = [[strPath pathExtension] uppercaseString];
        if (([extension isEqualToString:@"TTF"]) || ([extension isEqualToString:@"OTF"])) {
            NSString *fontFilename = [strPath lastPathComponent];
            if (fontFilename && ([fontFilename length] > 0) && (![fontFilename hasPrefix:@"."])) {
